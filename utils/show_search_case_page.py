...
def search_similar_case(str_phenotypes, str_genes):
    ...
    #### +--------------------+------------------+------+-----+---------+----------------+
    sql = u"select CaseID, IndexOntoIDHP, CaseOntoIDHP, CaseOntoIDHPSource, CommonRootHP, CommonRootHPIC from IndexCaseHP where IndexOntoIDHP in (%s) order by CaseID, CaseOntoIDHP"
    ...
    for value in values:
        ...
        if case_id in dict_similar_cases:
            ...
            if dict_IC[onto_id_hp_index] != 0:
                # GeneYenta: 分子
                dict_similar_cases[case_id]['sum_ic'] += ic
                # GeneYenta: 分母
                dict_similar_cases[case_id]['sum_ic_denominator'] += dict_IC[onto_id_hp_index]
            else:
                # GeneYenta: 分子
    ...
    # CaseGeneテーブルから各症例に紐づけられた遺伝子を取得
    dict_case_gene = {}
    ...
    # ユーザが指定した遺伝子を疾患原因遺伝子に持つ疾患をCaseGeneテーブルから取得
